...
import { buildSyncTasks, detectLang, removeExt, artifactRelativePathForCode, scanArtifactFiles, buildReport, } from "./artifact-sync.js";
function codeFile(relativePath, lang = "typescript") {
    return {
        lang: lang,
        relativePath,
...
describe("artifactRelativePathForCode", () => {
    it("maps code path to spec path", () => {
        expect(artifactRelativePathForCode("src/main.ts", "code-specs", ".md")).toBe("code-specs/src/main.md");
    });
    it("maps code path to VitePress Markdown path", () => {
        expect(artifactRelativePathForCode("src/main.ts", "docs", ".md")).toBe("docs/src/main.md");
    });
});
describe("scanArtifactFiles", () => {
    it("excludes locale directories while scanning VitePress Markdown", async () => {
        const projectPath = await mkdtemp(path.join(tmpdir(), "code-check-docs-"));
        const docsPath = path.join(projectPath, "docs");
        try {
            await mkdir(path.join(docsPath, "ru"), { recursive: true });
            await mkdir(path.join(docsPath, ".vitepress"), { recursive: true });
            await writeFile(path.join(docsPath, "index.md"), "# English", "utf-8");
            await writeFile(path.join(docsPath, "ru", "index.md"), "# Русский", "utf-8");
            await writeFile(path.join(docsPath, ".vitepress", "guide.md"), "# Config", "utf-8");
