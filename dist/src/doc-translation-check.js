...
export function ruPathForEn(enRelPath) {
    ...
    if (!normalizedPath.startsWith(docsPrefix) ||
...
export async function runDocTranslationCheck(projectPath) {
    ...
    for (const enFile of enFiles) {
        ...
        workflow = workflow.prompt(key, () => buildAgentPrompt(absoluteProject, enFile), { model: "code-check-model", schema: DecisionSchema });
    }
