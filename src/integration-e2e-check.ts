...
function hasTestsField(value: unknown): value is { tests: Array<{ path: string; description: string }> } {
...
function getTestFile(value: unknown): { path: string; description: string } | null {
    if (typeof value !== 'object' || value === null) {
        return null
    }
...
async function readFileIfExists(projectPath: string, relPath: string): Promise<string | null> {
    try {
...
async function ensureTestDir(projectPath: string, relPath: string): Promise<void> {
    const targetPath = path.join(projectPath, relPath)
    await mkdir(path.dirname(targetPath), { recursive: true })
}

function buildIntegrationPlanPrompt(
    projectPath: string,
    codeFiles: CodeFile[],
    specFiles: ArtifactFile[],
): string {
    return [
...
function buildIntegrationTestPrompt(
    projectPath: string,
    testPath: string,
    description: string,
    codeFiles: CodeFile[],
...
export async function runIntegrationE2eCheck(
    projectPath: string,
): Promise<Result<IntegrationE2eCheckReport, Error>> {
    const absoluteProject = path.resolve(projectPath)
    let codeFiles: CodeFile[]
    let specFiles: ArtifactFile[]
    let docFiles: ArtifactFile[]
    try {
    ...
    let workflow = weave<Record<string, unknown>>()
    ...
    const finalWorkflow = workflow.step('generate', async (ctx) => {
        ...
        const entries: IntegrationE2eReportEntry[] = []
        ...
        for (const test of integrationPlan.tests) {
            ...
            const testWorkflow = weave<Record<string, never>>()
                .prompt(
                    key,
