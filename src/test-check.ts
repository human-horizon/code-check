...
import { detectLang, type CodeFile } from "./artifact-sync.js"
...
function collectDecisions(
    ctx: object,
): Array<{ key: string; decision: TestDecision }> {
...
export async function runTestCheck(
    ...
    for (const code of codeFiles) {
        existingTestsMap.set(
            code.relativePath,
            await readTestFile(absoluteProject, code),
        )
    }
    ...
    for (const code of codeFiles) {
        ...
        workflow = workflow.prompt(
            ...
            { model: "code-check-model", schema: TestDecisionSchema, retry: 3 },
        )
    }
    ...
    const finalWorkflow = workflow.step("report", async (ctx) => {
        ...
        for (const { key, decision } of pairs) {
            ...
            const code = codeFiles.find(
                (c) => safeKey(c.relativePath) === codeRel,
            )
            if (!code) {
                errors.push({
            ...
            const existingTests =
                existingTestsMap.get(code.relativePath) ?? null
